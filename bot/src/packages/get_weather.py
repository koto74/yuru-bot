...
class WeatherInfo:
    ...
    async def send_rain_alert(self, send_channel_id):
        for city in self.city_id_map.keys():
            ...
            if "雨" in today_weather:
                await self.__send_rain_weather_details(
                    weather_data, city, send_channel_id
                )
    ...
    async def __send_rain_weather_details(self, weather_data, city, send_channel_id):
        today_weather_embed = discord.Embed(
            title="今日 " + city + " は雨が降るらしいよ！",
            color=0x00FF00,
            url="https://weather.yahoo.co.jp/weather/jp/13/4410.html",
        )
        today_weather_embed.add_field(
            name=weather_data["title"], value=weather_data["forecasts"][0]["telop"]
        )
